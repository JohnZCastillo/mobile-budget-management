...
export default function ModalScreen(){
    ...
    const handleOnAdd = async () => {
        try{
            ...
            const updateWallet = ()=> {
                return db.update(schema.walletTable)
                    .set({amount: sql`${schema.walletTable.amount} + ${amount}`})
                    .where(eq(schema.walletTable.id, 1));
            }

            const addIncome = ()=> {
                return db.insert(schema.income)
                         .values({
                            title: title,
                            amount: amount,
                            date: Date()
            ...
            Promise.all([updateWallet(), addIncome()]);
